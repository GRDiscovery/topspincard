...
_handleTongleSw = () => {
 ...
 if(this.state.switchValue == false){
  ...
  AsyncStorage.setItem('numcard', this.state.creditCard);
  AsyncStorage.setItem('lembreme', 'S');


 }else{
  AsyncStorage.removeItem('numcard');
  AsyncStorage.removeItem('lembreme');
 }
  ...
  componentDidMount() {  
    ...
    AsyncStorage.getItem('lembreme', (err, result) => {
      if(result == 'S'){
        AsyncStorage.getItem('numcard', (err, result) => {
          this.setState({
            creditCard: result,
            switchValue: true
            })  
  ...
  render() {
    ...
    <SafeAreaView style={styles.safeview}>
        ...
        <Form>
          ...
          <Item regular style={{ marginLeft: 30, marginRight: 30, borderRadius:10, marginTop:40, height:50, backgroundColor:'#FFF' }}>
              <TextInputMask style={{backgroundColor:"#ff0000", width:"90%", backgroundColor:'#FFF', color: '#646464', fontSize:21, marginLeft:10  }}
              type={'credit-card'}
              options={{
              obfuscated: false,
              }}
              ...
              />
              {/* <Input onChangeText={numcard => this.setState({ numcard })} value={this.state.numcard} autoCapitalize="none" keyboardType='number-pad' style={{ color: '#646464' }} stackedLabel picker placeholder="Número do cartão" placeholderTextColor='#646464' /> */}
          </Item>

          <Item regular style={{ marginLeft: 30, marginRight: 30, marginTop: 50, borderRadius:10, backgroundColor:'#FFF'  }}>
            <Input onChangeText={senha => this.setState({ senha })} value={this.state.senha} autoCapitalize="none"  secureTextEntry keyboardType='default' style={{ color: '#646464' }} stackedLabel picker placeholder="Senha" placeholderTextColor='#646464' />
          </Item>
